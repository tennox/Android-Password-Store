...
class AppPasskeyProviderActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    CoroutineScope(dispatcherProvider.mainImmediate()).launch {
  ...
  private suspend fun handleGetCredential(request: androidx.credentials.provider.ProviderGetCredentialRequest) {
    ...
    val parsedRequest =
      PasskeyProviderUtils.json.decodeFromString<app.passwordstore.passkeys.provider.WebAuthnGetRequest>(
        option.requestJson
      )

    val credentialId = PasskeyProviderUtils.decodeBase64Url(selectedCredentialId)
    val credential =
      passkeyStorage.getCredential(credentialId).fold(
        success = { it },
        failure = {
    ...
    if (authenticator.canAuthenticate(this)) {
      when (val authResult = authenticator.authenticateForPasskey(this, credential.rpId)) {
        ...
        is PasskeyAuthenticator.Result.Failure -> {
          finishWithGetError(GetCredentialUnknownException("Authentication failed: ${authResult.message}"))
          return
        }
      }
    }
    ...
    passkeyStorage.updateSignCount(credential.credentialId, newSignCount).fold(
      success = { },
  ...
  private suspend fun handleCreateCredential(request: androidx.credentials.provider.ProviderCreateCredentialRequest) {
    ...
    val parsedRequest =
      PasskeyProviderUtils.json.decodeFromString<app.passwordstore.passkeys.provider.WebAuthnCreateRequest>(createRequest.requestJson)
    ...
    if (authenticator.canAuthenticate(this)) {
      when (val authResult = authenticator.authenticateForCreation(this, parsedRequest.rp.id)) {
        ...
        is PasskeyAuthenticator.Result.Failure -> {
          finishWithCreateError(CreateCredentialUnknownException("Authentication failed: ${authResult.message}"))
          return
        }
      }
    }
    ...
    val responseJson = PasskeyProviderUtils.buildAttestationResponse(createdCredential, createRequest.requestJson)
    val resultIntent = Intent()
  ...
  private fun finishWithGetError(exception: androidx.credentials.exceptions.GetCredentialException) {
    val resultIntent = Intent()
